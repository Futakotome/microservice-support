...
public class BatchStream implements ReadStream<Batch>, WriteStream<Batch> {
    ...
    public BatchStream(ReadStream<Buffer> rs, WriteStream<Buffer> ws) {
        Objects.requireNonNull(rs, "ReadStream must not be null");
        Objects.requireNonNull(ws, "WriteStream must not be null");

        recordParser = RecordParser.newFixed(4, rs);
        writeStream = ws;
        recordParser.exceptionHandler(event -> {
            if (exceptionHandler != null) {
                exceptionHandler.handle(event);
            }
        });
        writeStream.exceptionHandler(event -> {
            if (exceptionHandler != null) {
                exceptionHandler.handle(event);
            }
        });
